    ...
    <AnimatePresence>
      {open ? (
        <motion.div
          ...
          className="dialog-backdrop fixed inset-0 z-[70] overflow-y-auto px-3 py-4 sm:px-6 sm:py-8"
          onClick={(event) => {
        ...
        >
          <div className="flex min-h-full items-center justify-center">
            <motion.div
              ...
              className="dialog-shell relative flex max-h-[min(92vh,900px)] w-full max-w-[1040px] flex-col overflow-hidden rounded-[30px]"
            >
              ...
              <div className="custom-scrollbar scroll-stable flex-1 overflow-y-auto px-4 pb-5 pt-4 sm:px-6">
                  ...
                  {headerStats.map((item) => {
                    ...
                    return (
                      <div
                        key={item.label}
                        className={cn(
                          "dialog-section-muted flex items-start gap-3",
                      ...
                      <div className="mt-4 grid gap-4 lg:grid-cols-[minmax(0,1fr)_280px]">
                        ...
                        <div className="space-y-3 rounded-[22px] border border-slate-200/85 bg-slate-50/82 p-4 dark:border-slate-700/50 dark:bg-slate-900/55">
                          ...
                          <p className="text-xs leading-5 text-slate-500 dark:text-slate-400">
                            Use este campo para já deixar o próximo passo visível na carteira e no dashboard.
                          </p>
                ...
                <section className="dialog-section mt-5">
                    ...
                    <div className="mt-4 space-y-4">
                      {groupedTimeline.map(([groupLabel, entries]) => (
                        <div key={groupLabel} className="space-y-3">
                          <div className="sticky top-0 z-[1] inline-flex rounded-full border border-slate-200 bg-white/95 px-3 py-1 text-xs font-medium uppercase tracking-[0.16em] text-slate-500 shadow-[0_12px_24px_-22px_rgba(15,23,42,0.18)] dark:border-slate-700/55 dark:bg-slate-950/85 dark:text-slate-300">
                            {groupLabel}
                          </div>
                          {entries.map((entry) => (
